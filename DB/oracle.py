...
if __name__ == '__main__':
    ...
    rows = exec_Oracle_SQL( cur, "select EMPNAME,EMPCODE,EMPCODE2,EIGYOBI, TENPOCODE from T_HIBETUSFTHEAD_SIME" )
    ...
    for row in rows:
        ...
        cur.execute("update T_HIBETUSFTHEAD_SIME set empname = '" + newname + "'  \
                            where EMPCODE = '" + str(row[1]) + "' and EMPCODE2 = '" + row[2] + "' and EIGYOBI = '" + row[3] + "' " \
                            + " and TENPOCODE = '" + str(row[4]) + "' " )
    conn.commit()
    print('The end ...')  