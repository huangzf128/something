...
class FindFile(base_file.BaseFile):
    ...
    def get_all_file(self, d_names=None, exclude_d_names=None):
        """ get all files path in d_names
        """
        return self.get_file_by_name(None, d_names, exclude_d_names)

    def get_file_by_name(self, f_names, d_names=None, exclude_d_names=None):
        """     f_names: None for all. eg: root/a/b/c/d
    ...
    def get_file_by_name_reg(self, f_name_patterns, d_names=None, exclude_d_names=None):
        """     f_name_patterns: None for all. eg: root/a/b/c/d
                d_names: more level more accuracy. "/" must at head and tail.  eg: /a/b/, /b/, /a/b/c/
    ...
    def get_file_by_modifiedtime(self, modifiedtime, compare_type, exclude_d_names=None):
        ...
        for path, subdirs, files in os.walk(self.search_in_dir):
            ...
            for name in files:
                ...
                if ((compare_type == self.COMPARE_EQUALS and updatetime == modifiedtime) or
                        (compare_type == self.COMPARE_GREANTER and updatetime > modifiedtime) or
                        (compare_type == self.COMPARE_LESS and updatetime < modifiedtime)):
